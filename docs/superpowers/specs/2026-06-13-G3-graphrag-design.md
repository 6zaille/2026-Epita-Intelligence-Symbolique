...
- Clic sur le chip → drawer animé (CSS transition `max-height`) révélant :
  - Étapes de raisonnement numérotées (nœud de départ, hop 1, hop 2…) avec entities tags
...
- Barre d'input bloquée (disabled + badge explicatif) pendant les états ① et ② :
  ...
  - ② : badge "Construction du KG en cours…" + barre de progression SSE
...
- Animations : chip drawer (CSS), barre de progression (SSE), dot "live" pulsant (Leiden en cours)
...
### `benchmark.ipynb` (nouveau)
- Chargement d'un subset HotpotQA (questions multi-hop)
